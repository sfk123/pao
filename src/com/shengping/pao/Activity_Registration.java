 ...
 *
 */
public class Activity_Registration  extends Activity implements OnClickListener,DrawableRightListener{
	...
	public void onClick(View v) {
		if(v.getId()==R.id.btn_back){
			...
			if(start==null){
				MyUtil.alert("请选择到哪里挂号", this);
				return;
			}else if(tv_phone.getText().toString().trim().equals("")){
				MyUtil.alert("请输入联系电话", this);
				return;
			}else if(tv_message.getText().toString().trim().equals("")){
				MyUtil.alert("请输入挂号具体事项", this);
				return;
			}else{
				Intent intent=new Intent(this,Activity_Registration_confirm.class);
