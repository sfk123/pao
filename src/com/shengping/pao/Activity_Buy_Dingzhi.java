...
public class Activity_Buy_Dingzhi  extends Activity implements OnClickListener,DrawableRightListener{
	
	private EditText location_start_select,edt_price;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_buy_dingzhi);
		ImageView btn_back=(ImageView)findViewById(R.id.btn_back);
		btn_back.setOnClickListener(this);
		TextView tv_title=(TextView)findViewById(R.id.tv_title);
		tv_title.setText("订制购买");
	...
	public void onClick(View v) {
		if(v.getId()==R.id.btn_back){
			...
			intent.putExtra("title", "去哪买");
			startActivityForResult(intent, request_start);
		}else if(v.getId()==R.id.location_end_select){
			Intent intent=new Intent(this,Activity_SelectLocation.class);
			intent.putExtra("current_location", location_start_select.getText().toString());
			intent.putExtra("type", "end");
			intent.putExtra("title", "送往...");
			startActivityForResult(intent, request_start);
		}else if(v.getId()==R.id.btn_confirm){
			if(edt_price.getText().toString().trim().equals("")){
				MyUtil.alert("请输入商品价格", this);
				return;
			}else if(start==null){
				MyUtil.alert("请选择出发地", this);
				return;
			}else if(end==null){
				MyUtil.alert("请选择目的地", this);
				return;
			}else if(tv_phone.getText().toString().trim().equals("")){
				MyUtil.alert("请输入收货电话", this);
				return;
			}else{
				Intent intent=new Intent(this,Activity_Buy_Dingzhi_Confirm.class);
				...
				intent.putExtra("price", edt_price.getText().toString());
